...
import { setCustomInterval, clearCustomInterval } from './utils.js';
...
wss.on('connection', (ws) => {
    ...
    if (pid === -1) { return; } // already 2 players playing
    ...
    ws.on('message', (data) => {
        ...
        case 'pong':
            ...
            break;
        case 'packet': {
            const packet = (message.payload.packet as Packet);
    ...
    lc.forOthers(pid, (other: PlayerInfo) => {
...
setCustomInterval(() => {
    ...
    lc.playerArr.forEach((player: PlayerInfo | null, pid: number) => {
        if (!player) { return; }
