import { Container, Graphics, Point } from '../../pixi.mjs';
...
export class PlayerContainer extends Container {
    ...
    checkHitscanOn(enemyContainer) {
        // laser definition
        const theta = this.rotation;
        const origin = this.getChildByLabel('laser').getGlobalPosition(); // this is immediately updated, unlike worldTransform
        // check if laser hits each edge
        let doesHit = false;
        let hitPointG = new Point(-1, -1);
        let minDist = Infinity;
        const hitbox = enemyContainer.getChildByLabel('hitbox');
        for (let i = 1; i < 5; i++) {
            const va = enemyContainer.localTransform.apply(new Point(hitbox[`e${i}`].va.x, hitbox[`e${i}`].va.y));
            const vb = enemyContainer.localTransform.apply(new Point(hitbox[`e${i}`].vb.x, hitbox[`e${i}`].vb.y));
    ...
    #checkRayIntersectSegment(theta, origin, p1, p2) {
        // Ray Line Equation
